...
int controller_filtrarPorGenero(LinkedList* pArrayPeliculas)
{
	int todoOk=0;
	...
	if(pArrayPeliculas != NULL)
	{
		...
		{
		case 1:
			auxList = ll_filter(pArrayPeliculas, peliculas_filtrarPeliculaDrama);
			break;
		case 2:
			auxList = ll_filter(pArrayPeliculas, peliculas_filtrarPeliculaComedy);
			break;
		case 3:
			auxList = ll_filter(pArrayPeliculas, peliculas_filtrarPeliculaRomance);
			break;
		case 4:
			auxList = ll_filter(pArrayPeliculas, peliculas_filtrarPeliculaAdventure);
			break;
		default:
		...
		if( auxList != NULL)
		{
			...
			if(controller_guardarComoTexto("peliculasGenero.csv", auxList))
			{
				controller_listarPeliculas(auxList);
			}
			else
		...
		{
			printf("Error al filtrar los paises! \n");
		}
		todoOk=1;
	}
	return todoOk;
